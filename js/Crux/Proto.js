Proto = {
	...
	instanceFromDict: function(dict)
	{
		//console.log("Proto.instanceFromDict(" + JSON.stringify(dict) + ")")
		
		var protoName = dict.type;
		
		if (protoName == null)
		{
			throw("Proto.instanceFromDict() unable to find type in dict '" + JSON.stringify(dict) + "'")
		}
		
		var proto = globals[protoName];
		
		if (proto == null)
		{
			throw("Proto.instanceFromDict() unable to find type '" + protoName + "'")
		}
		
		var instance = proto.clone()
		
		instance.initWithDict(dict);
		return instance
	}
}
